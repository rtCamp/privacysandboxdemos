...
function scoreAd(
  ...
  directFromSellerSignals,
	...
	var score = bid+10;
  var adjustedBid = bid-2; /* simulate net adjustment */

	console.groupCollapsed( "%c PAAPI SSP1-ComponentSeller %c scoreAd %c BID:%d => SCORE:%d, SUBMITTED:%d", "color: green;  background-color:yellow; border: 1px solid black", "color: blue; border:1px solid black", "", bid, score, adjustedBid);
...
function reportResult(auctionConfig, browserSignals, directFromSellerSignals) {
	console.groupEnd();
	console.groupCollapsed( "%c PAAPI SSP1-ComponentSeller %c reportResult %c BID:%d", "color: green;  background-color:yellow; border: 1px solid black", "color: blue; border:1px solid black", "", browserSignals.bid);
